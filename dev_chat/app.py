    ...
    for msg in st.session_state.messages:
        ...
        if role == "assistant":
            ...
            with st.container(border=True):
                ...
                st.text_area(
                    "Pre-filled prompt (copy → paste into Gemini):", 
                    value=prompt_text, height=120)
                st.caption(
                    "This prompt was generated by HelpAgent based on your specific struggle.")
...
def render_input_area() -> None:
    ...
    if phase == "idle":
        ...
        with col1:
            uid = st.text_input(
                "Learner UID", 
                value=st.session_state.uid, key="uid_input", 
                label_visibility="collapsed", 
                placeholder="Learner UID (e.g. test-user-dev)")
        with col2:
        ...
        if question is None:
            ...
            if fmt == "tf":
                col1, col2 = st.columns(2)
                with col1:
                    if st.button(options[0] if options else "True", 
                                use_container_width=True):
                        action_submit_answer(options[0] if options else "True")
                        st.rerun()
                with col2:
                    if st.button(options[1] if len(options) > 1 else "False", 
                                use_container_width=True):
