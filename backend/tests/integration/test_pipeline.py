...
class TestMainSessionStart:
    ...
    def _make_context_event(self, json_payload: str | None = None) -> MagicMock:
        """Build a mock final event carrying a context_output JSON payload."""
        event = MagicMock()
        ...
        if json_payload is None:
            json_payload = json.dumps({
                ...
                "session_goal": "Understand what Linux is",
            })
        part = MagicMock()
    ...
    async def test_session_start_returns_session_id(self) -> None:
        ...
        body = response.json()
        # session_id is now a uuid4 generated by main.py (not the ADK session id)
